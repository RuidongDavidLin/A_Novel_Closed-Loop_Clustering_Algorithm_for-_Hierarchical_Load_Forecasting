...
def train_model(cluster_num, cluster_list, aggr_data, lag, train_ratio, temp_data, time_index):
    """
    ...
    for index in range(cluster_num):
        ...
        if counts[index] == 0:
            ...
            x_data = aggr_data[index][:-lag].reshape(-1,1)
            x_data = np.concatenate((x_data,temp_data[:-lag].reshape(-1,1)),axis=1)
            x_data = np.concatenate((x_data,time_index[lag:].reshape(-1,1)),axis=1)
            x_data = np.concatenate((x_data,(time_index[lag:]**2).reshape(-1,1)),axis=1)
            x_data = np.concatenate((x_data,np.sqrt(time_index[lag:]).reshape(-1,1)),axis=1)
            
            y_data = aggr_data[index][lag:]
...
def error_feedback(load_data, cluster_num, model_dict, lag, train_ratio, val_ratio, temp_data, time_index):
    matrix = np.full((load_data.shape[0],cluster_num), 1e9)
    for index in range(cluster_num):
        for user_id in range(load_data.shape[0]):
            if model_dict[str(index)] != None:
                x_data = load_data[user_id][:-lag].reshape(-1,1)
                x_data = np.concatenate((x_data,temp_data[:-lag].reshape(-1,1)),axis=1)
                x_data = np.concatenate((x_data,time_index[lag:].reshape(-1,1)),axis=1)
                x_data = np.concatenate((x_data,(time_index[lag:]**2).reshape(-1,1)),axis=1)
                x_data = np.concatenate((x_data,np.sqrt(time_index[lag:]).reshape(-1,1)),axis=1)
                y_data = load_data[user_id][lag:]
                
                x_train = x_data[int(train_ratio*x_data.shape[0]):int((train_ratio+val_ratio)*x_data.shape[0])]
                y_train = y_data[int(train_ratio*y_data.shape[0]):int((train_ratio+val_ratio)*y_data.shape[0])]
...
def Predict(cluster_num, cluster_list, aggr_data, lag, train_ratio, val_ratio, temp_data, time_index, model_dict):
    """
...
def Predict_val(cluster_num, cluster_list, aggr_data, lag, train_ratio, val_ratio, temp_data, time_index, model_dict):
    """
    cluster_num: 聚类簇数 num of clulsters \n
    cluster_list: 每个用户所属簇列表 list including cluster_id of each user \n
    aggr_data: 每个簇聚合后负荷 aggregated load curve of each clusters \n
